...
if st.button("🔍 Calcular Cenários"):
    ...
    for prazo in range(1, 97):
        ...
        for _ in range(100): # This loop was incorrectly indented
            mid = (low + high) / 2
    ...
    if melhor_cenario2:
        ...
        st.info(f"↔️ Diferença para Total Original: **R$ {mel_hor_cenario2['diferenca']:,.2f}**".replace(",", "X").replace(".", ",").replace("X", "."))
